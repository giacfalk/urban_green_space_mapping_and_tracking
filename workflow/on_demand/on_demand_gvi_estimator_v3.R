...
setwd("F:/.shortcut-targets-by-id/13znqeVDfPULc4J_lQLbyW_Kmfa03o63F/3-Research/GIACOMO/systemic cooling poverty/glob_assess_energy_demand_reduction_potential")
...
# lon_x = 0; lat_y = 55.5; obs_id = 1; year_yr = 2019; buffer_size = 500
...
ndvi_2016 = ee_extract(yrMo, p, fun=ee$Reducer$median(), scale=buffer_size, sf=F, via="drive", lazy=F)
...
landcover_2016 = ee_extract(dwImage, p, fun=ee$Reducer$median(), scale=buffer_size, sf=F, via="drive", lazy=F)
...
era_2016 = ee_extract(era5, p, fun=ee$Reducer$median(), scale=buffer_size, sf=F, via="drive", lazy=F)
...
pop = ee_extract(dwImage, p, fun=ee$Reducer$median(), scale=buffer_size, sf=F, via="drive", lazy=F)
...
# load("C:/Users/Utente/OneDrive - IIASA/Current papers/greening/urban_green_space_mapping_and_tracking/data/validation/after_points_step1.Rdata")

pr_2016 <- h2o::h2o.predict(saved_model,  as.h2o(pr_data_2016[complete.cases(pr_data_2016),]))

pr_2016 <- exp(as.data.frame(pr_2016)$predict)
